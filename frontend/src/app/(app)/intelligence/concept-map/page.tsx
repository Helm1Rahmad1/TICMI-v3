    ...
    async function loadMap() {
      try {
        ...
        if (response.ok) {
          ...
          const activeNodeId = data.studentMemory?.prerequisite_gaps?.[0] || 'f-fungsi-komposisi-invers';
          
          // Find which track contains this active node
          const containingTrack = Object.keys(PATHS).find(trackId => PATHS[trackId].includes(activeNodeId));
          if (containingTrack) {
            setSelectedTrack(containingTrack);
          }

          const activeNode = data.nodes.find((n: ConceptNode) => n.id === activeNodeId);
