...
function Node({ x, y, r, fill, label, ring, focus, locked }: {
  x:number; y:number; r:number; fill:string; label:string; ring?:boolean; focus?:string; locked?:boolean;
}) {
  return (
    <g>
      ...
      <text x={x} y={y + r + 18} textAnchor="middle" fontSize="11" fill="#14141A" fontWeight={focus ? 600 : 400} opacity={locked ? 0.5 : 1}>{label}</text>
    </g>
  );
}
      ...
      <div style={{ padding: '12px 18px 4px' }}>
        <div style={{ padding: '14px', borderRadius: 18, background: '#fff', border: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'var(--f-mono)', fontSize: 9.5, letterSpacing: '.1em', color: 'var(--muted)' }}>FOKUS SEKARANG</div>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 10, lineHeight: 1.5 }}>
            Selesaikan ini dan <b>invers matriks</b> terbuka. Kiko sudah paham resiprokal — stuck di n=0.
          </div>
          <button onClick={() => router.push('/student/teach-me')} style={{ marginTop: 12, width: '100%', padding: '14px', background: 'var(--grad)', color: '#fff', border: 0, borderRadius: 14, fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 10px 24px -8px rgba(91,91,247,.5)', cursor: 'pointer' }}>
